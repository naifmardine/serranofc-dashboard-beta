...
// ✅ form com suporte a clubeId/clubeRef (mesmo se o type Jogador ainda não tiver)
type JogadorForm = Partial<Jogador> & {
  clubeId?: string | null;
  clubeRef?: { id: string; nome: string; slug?: string; logoUrl?: string | null } | null;
};

function createEmptyForm(): JogadorForm {
  return {
    nome: "",
    idade: null,
    posicao: "ATA",
...
export default function NovoJogadorPage() {
  ...
  const [form, setForm] = useState<JogadorForm>(() => createEmptyForm());
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // se você quiser permitir jogador sem clube, remove esse bloco
      if (!form.clubeId) {
        throw new Error("Selecione um clube cadastrado.");
      }

      // não mandar clubeRef pro backend
      ...
      // compatível com vários formatos: { jogador }, { player }, ou o próprio objeto
      const created =
        data?.jogador ?? data?.player ?? (data?.id ? data : null);
  ...
  return (
    <>
      <SuccessDialog
        ...
        onPrimary={() => {
          setSuccessMsg(null);
          if (createdId) router.push(`/admin/jogadores`);
        }}
      />
      ...
      <section className="p-6 max-w-5xl mx-auto">
        <div className="mb-5 flex items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-xl font-bold text-slate-900">Novo jogador</h1>
          </div>
                ...
                <div className="space-y-2">
                  <ClubSelect
                    ...
                    onChange={(club) => {
                      ...
                      updateField("clube", club?.nome ?? ""); // legado
                    }}
